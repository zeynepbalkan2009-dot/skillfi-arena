...
type PortalData = { studio: Studio | null; games: Game[]; reviewFeedback: Array<{ gameId: string | null; eventType: string; note: string; createdAt: string }>; isAdmin: boolean; fee: { amount: string; displayAmount: string; treasury: `0x${string}` } };
...
export function StudioPortalClient() {
  ...
  async function updateGame(event: FormEvent) {
    ...
    try {
      const token = await getAccessToken();
      const response = await fetch("/api/studios/games", { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ gameId: editingGame.id, name: editingGame.name, type: editingGame.type, description: editingGame.description, websiteUrl: editingGame.websiteUrl }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.error ?? "Could not update game draft");
      setEditingGame(null); await load(); setMessage("Game draft updated. You can submit it for review again.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not update game draft"); }
    finally { setBusy(false); }
  }
      ...
      <div className="mx-auto max-w-4xl">
        ...
        {!authenticated ? <div className="mt-8 rounded-xl border border-arena-border bg-arena-surface p-6">Connect and log in to create a studio.</div> : !data ? <p className="mt-8 text-arena-muted">Loading…</p> : !data.studio ? (
          ...
          <div className="mt-8 grid gap-6">
            ...
            {data.games.length > 0 && <section className="rounded-xl border border-arena-border bg-arena-surface p-6"><h2 className="font-display text-xl font-bold">Your games</h2><div className="mt-4 space-y-3">{data.games.map((game) => <div key={game.id} className="rounded-md border border-arena-border bg-arena-bg p-4"><div className="flex justify-between gap-3"><span className="font-semibold">{game.name}</span><span className="text-sm capitalize text-arena-muted">{game.integration_status}</span></div><p className="mt-2 text-sm text-arena-muted">{game.description}</p>{data.reviewFeedback.find((feedback) => feedback.gameId === game.id) && <p className="mt-3 rounded-md border border-arena-danger/40 bg-arena-danger/10 p-3 text-sm text-arena-danger">Review feedback: {data.reviewFeedback.find((feedback) => feedback.gameId === game.id)?.note}</p>}{editingGame?.id === game.id ? <form onSubmit={updateGame} className="mt-4 grid gap-3"><input required minLength={2} maxLength={100} value={editingGame.name} onChange={(event) => setEditingGame({ ...editingGame, name: event.target.value })} className="rounded-md border border-arena-border bg-arena-surface px-3 py-2" /><select value={editingGame.type} onChange={(event) => setEditingGame({ ...editingGame, type: event.target.value })} className="rounded-md border border-arena-border bg-arena-surface px-3 py-2"><option value="web2">Web2</option><option value="web3">Web3</option></select><textarea required maxLength={1000} value={editingGame.description} onChange={(event) => setEditingGame({ ...editingGame, description: event.target.value })} className="min-h-24 rounded-md border border-arena-border bg-arena-surface px-3 py-2" /><input type="url" placeholder="Game website (optional)" value={editingGame.websiteUrl} onChange={(event) => setEditingGame({ ...editingGame, websiteUrl: event.target.value })} className="rounded-md border border-arena-border bg-arena-surface px-3 py-2" /><div className="flex gap-2"><button disabled={busy} className="rounded-md bg-arena-accent px-3 py-2 text-xs font-semibold text-arena-bg">Save changes</button><button type="button" onClick={() => setEditingGame(null)} className="rounded-md border border-arena-border px-3 py-2 text-xs">Cancel</button></div></form> : <div className="mt-3 flex flex-wrap gap-2">{['draft', 'rejected'].includes(game.integration_status ?? "") && <button type="button" onClick={() => setEditingGame({ id: game.id, name: game.name, type: game.type, description: game.description ?? "", websiteUrl: game.website_url ?? "" })} disabled={busy} className="rounded-md border border-arena-border px-3 py-2 text-xs text-arena-muted">Edit draft</button>}{game.integration_status === "draft" && <button type="button" onClick={() => submitGame(game.id)} disabled={busy || data.studio?.status === "pending_payment"} className="rounded-md border border-arena-accent-dim px-3 py-2 text-xs font-semibold text-arena-accent disabled:opacity-40">Submit for review</button>}{['sandbox', 'published'].includes(game.integration_status ?? "") && <button type="button" onClick={() => createCredential(game)} disabled={busy} className="rounded-md border border-arena-accent-dim px-3 py-2 text-xs font-semibold text-arena-accent disabled:opacity-40">Create integration key</button>}</div>}</div>)}</div></section>}
