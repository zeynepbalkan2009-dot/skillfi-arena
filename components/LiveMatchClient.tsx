...
export function LiveMatchClient({ match: initialMatch }: { match: MatchView }) {
  ...
  const startedAtRef = useRef<number | null>(
    initialMatch.started_at
      ? new Date(initialMatch.started_at).getTime()
      : null,
  );
  ...
  const pilotRound = useMemo(
    () =>
      pilotGameId
        ? createPilotRound(pilotGameId, initialMatch.smart_contract_match_id)
        : null,
  ...
  async function disputeMatch() {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(body.error ?? "Dispute indexing failed");
      setMatch((current) => ({ ...current, status: "disputed" }));
      setDisputeOpen(false);
      setResult(
        "Match disputed. Automatic settlement is paused for arbiter review.",
      );
    } catch (error) {
  ...
  const submitResult = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("/api/matches/result", {
        ...
        body: JSON.stringify({
          matchId: match.smart_contract_match_id,
          answer: text,
          elapsedMs,
        }),
      });
      const body = await response.json().catch(() => ({}));
      if (!response.ok)
        throw new Error(body.error ?? "Result submission failed");
  ...
  useEffect(() => {
    ...
    async function refreshMatchState() {
      const { data, error } = await supabase
        .from("matches")
        .select(
          "status,winner_id,started_at,player_a_id,player_b_id,updated_at",
        )
        .eq("id", match.id)
        .maybeSingle();
  ...
  useEffect(() => {
    ...
    const tick = () =>
      setRemaining(
        Math.max(0, 60000 - (Date.now() - (startedAtRef.current as number))),
      );
    tick();
  ...
  const typingScore = scoreTyping(
    passage,
    text,
    Math.max(1000, 60000 - remaining),
  );
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-sm text-arena-muted">
              {match.player_a?.username ?? "Player 1"} vs{" "}
              {match.player_b?.username ?? "Waiting..."}
            </p>
          </div>
        </div>
        ...
        {match.status === "searching" && (
          <div className="rounded-xl border border-arena-border bg-arena-surface p-10 text-center">
            <WaitingMotion label="Waiting for Player 2" />
            <p className="mt-5 text-sm text-arena-muted">
              Keep this page open. The match starts automatically when another
              pilot player joins.
            </p>
          </div>
        )}
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_280px]">
            <section className="rounded-xl border border-arena-border bg-arena-surface p-6">
              <p className="mb-4 text-lg leading-8 text-arena-muted">
                {pilotRound && pilotGameId !== "typing-sprint"
                  ? challengePrompt
                  : challengePrompt.split("").map((char, index) => (
                      <span
                        key={index}
                        className={
                          index < text.length
              ...
              <textarea
                ...
                onChange={(event) => {
                  if (
                    (!pilotRound ||
                      pilotGameId !== "typing-sprint" ||
                      event.target.value.length <= challengePrompt.length) &&
                    !submitted
                  ) {
              ...
              <div className="mt-4 flex items-center justify-between">
                <div className="flex gap-5 text-sm text-arena-muted">
                  <span>
                    {pilotScore ? "Score" : "WPM"}{" "}
                    <b className="text-arena-text">
                      {pilotScore?.points ?? typingScore.wpm.toFixed(0)}
                    </b>
                  </span>
                  <span>
                    Accuracy{" "}
                    <b className="text-arena-text">
                      {pilotScore?.percent ??
                        (typingScore.accuracy * 100).toFixed(0)}
                      %
                    </b>
                  </span>
                  <span>
                    Correct{" "}
                    <b className="text-arena-text">
                      {pilotScore?.points ?? typingScore.correctChars}
                    </b>
                  </span>
                </div>
                <button
                ...
                {!disputeOpen ? (
                  ...
                  <div className="rounded-lg border border-arena-danger/30 bg-arena-danger/5 p-4 text-left">
                    ...
                    <p className="mt-1 text-xs text-arena-muted">
                      This note becomes part of the match audit trail.
                      Submitting also requires a wallet transaction.
                    </p>
                    <textarea
                      id="dispute-reason"
                      value={disputeReason}
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          disabled={
                            disputeBusy || disputeReason.trim().length < 10
                          }
            ...
            <aside className="rounded-xl border border-arena-border bg-arena-surface p-5">
              ...
              <p className="mt-1 text-arena-muted">
                {match.player_b?.username ?? "Player 2"}
              </p>
              <div className="mt-6 rounded-lg border border-arena-border bg-arena-bg/50 p-4">
                <p className="text-xs uppercase tracking-wide text-arena-muted">
                  Match integrity
                </p>
                <p className="mt-2 text-sm text-arena-muted">
                  Opponent progress is hidden until an authoritative result is
                  recorded. Match state is refreshed from the database using a
        ...
        {match.status === "completed" && (
          <div className="rounded-xl border border-arena-border bg-arena-surface p-10 text-center">
            <h2 className="font-display text-3xl font-bold">
              {match.winner_id ===
              (isPlayerA ? match.player_a_id : match.player_b_id)
                ? "YOU WIN"
                : "YOU LOSE"}
            </h2>
            <p className="mt-3 text-arena-muted">
              The result has been settled on-chain.
            </p>
          </div>
        )}
        ...
        {match.status === "disputed" && (
          <div className="rounded-xl border border-arena-danger/40 bg-arena-danger/10 p-10 text-center">
            <h2 className="font-display text-3xl font-bold text-arena-danger">
              MATCH DISPUTED
            </h2>
            <p className="mt-3 text-arena-muted">
              Automatic settlement is paused. An authorized arbiter must review
              the result.
            </p>
          </div>
        )}
