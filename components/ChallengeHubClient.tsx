...
export function ChallengeHubClient({ initialMatches, games }: { initialMatches: MatchWithRelations[]; games: Game[] }) {
  ...
  <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_320px]"><section className="rounded-3xl border border-white/7 bg-white/[.025] p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-display text-2xl font-bold">Open lobbies</h2><p className="mt-1 text-xs text-slate-600">Verified results · Testnet-only sessions</p></div><span className="rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-xs font-bold text-emerald-300">MATCHMAKING ONLINE</span></div>{needsProfile?<OnboardingCard/>:sorted.length===0?<div className="rounded-2xl border border-dashed border-white/10 py-20 text-center"><p className="font-display text-2xl font-bold text-white">The arena is quiet</p><p className="mt-2 text-sm text-slate-500">Be the first active pilot player to open a challenge.</p></div>:<div className="space-y-3">{sorted.map(match=><ChallengeCard key={match.id} match={match} isOwnChallenge={match.player_a_id===profile?.id}/>)}</div>}</section><aside className="rounded-3xl border border-indigo-400/15 bg-indigo-500/[.055] p-5"><p className="text-[10px] font-bold uppercase tracking-[.2em] text-indigo-300">Guild mission preview</p><h2 className="mt-2 font-display text-2xl font-bold">Voidrunners Hunt</h2><p className="mt-3 text-sm leading-6 text-slate-500">A non-financial test mission: win 3 verified matches against a rival guild before Sunday.</p><div className="mt-6 rounded-xl bg-black/20 p-4"><p className="text-xs text-slate-600">PILOT RECOGNITION</p><p className="mt-1 font-display text-2xl font-bold text-white">Guild badge</p></div><div className="mt-4 text-xs text-slate-500">Demo proposal · 16/24 wins simulated</div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/5"><div className="h-full w-2/3 bg-indigo-400"/></div></aside></div></main><CreateChallengeModal open={modalOpen} onClose={()=>setModalOpen(false)} games={games} currentUser={profile}/></GameShell>;
}
