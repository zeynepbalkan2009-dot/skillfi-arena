...
export function LobbyClient({
  ...
  useEffect(() => {
    async function refreshOpenMatches() {
      ...
      if (!openResponse.ok) {
        console.error(
          "Failed to refresh open matches:",
          openBody.error ?? openResponse.statusText,
        );
        return;
      }
      ...
      if (currentUser) {
        ...
        if (ownResponse.ok) {
          const pendingOwn = (
            (ownBody.matches as MatchWithRelations[] | null) ?? []
          ).filter(
            (match) =>
              match.player_a_id === currentUser.id &&
              match.status === "waiting_on_chain",
          );
          nextMatches = [
            ...pendingOwn,
            ...nextMatches.filter(
              (match) => !pendingOwn.some((own) => own.id === match.id),
            ),
          ];
        }
      }
    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-40 border-b border-white/5 bg-arena-bg/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 sm:px-8">
          ...
          >
            <span className="grid h-9 w-9 place-items-center rounded-lg border border-arena-accent/30 bg-arena-accent/10 font-display text-lg font-bold text-arena-accent shadow-arena-glow">
              S
            </span>
            <div>
              <p className="font-display text-lg font-bold leading-none tracking-[0.12em] text-white">
                SKILLFI
              </p>
              <p className="mt-1 text-[10px] uppercase tracking-[0.28em] text-arena-muted">
                Arena
              </p>
            </div>
          </Link>
          ...
          >
            ...
            <a href="#arena" className="transition hover:text-white">
              Live arena
            </a>
          ...
          <div className="flex items-center gap-3">
            {currentUser && (
              <>
                <Link
                  href="/studio"
                  className="hidden text-sm font-medium text-arena-muted hover:text-arena-text sm:block"
                >
                  For studios
                </Link>
                <Link
                  href="/profile"
                  className="text-sm font-medium text-arena-muted hover:text-arena-text"
                >
                  Profile
                </Link>
              </>
      ...
      <main
        id="arena"
        className="mx-auto max-w-5xl scroll-mt-20 px-5 py-24 sm:px-8"
      >
          ...
          <div>
            ...
            <p className="mt-3 text-arena-muted">
              {currentUser
                ...
                : "Connect to create or join a testnet challenge."}
            </p>
          </div>
        </div>
            ...
            <>
              <div className="mb-6 flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-arena-muted">
                    Create a stake-backed live match and wait for an opponent.
                  </p>
                </div>
                <button
                  type="button"
              ...
              {sortedMatches.length === 0 ? (
                <div className="rounded-lg border border-dashed border-arena-border py-16 text-center text-arena-muted">
                  <WaitingMotion compact label="No open challenges right now" />
                </div>
              ) : (
                <div className="space-y-3">
                  {sortedMatches.map((match) => (
                    <ChallengeCard
                      key={match.id}
                      match={match}
                      isOwnChallenge={match.player_a_id === currentUser?.id}
                    />
                  ))}
                </div>
              )}
      ...
      <footer className="border-t border-white/5">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-5 py-8 text-xs text-arena-muted sm:flex-row sm:items-center sm:justify-between sm:px-8">
          <p>
            © {new Date().getFullYear()} SkillFi Arena. Built for verifiable
            competition.
          </p>
