...
export function ChallengeCard({
  ...
  const region = match.player_a
    ? (REGION_LABELS[match.player_a.region] ?? match.player_a.region)
    : "-";
  const matchId = match.smart_contract_match_id ?? match.id;

  return (
    <article className="flex flex-col gap-5 border-t border-arena-border py-5 transition-colors first:border-t-0 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-4">
        <div className="flex h-10 w-10 items-center justify-center border border-arena-border bg-arena-bg font-mono text-xs text-arena-accent">
        ...
        <div>
          ...
          <p className="text-sm text-arena-muted">
            {match.player_a?.username ?? "Anonymous"} / {region} /{" "}
            {timeAgo(match.created_at)}
          </p>
          {match.status === "waiting_on_chain" && (
            <p className="mt-1 text-xs text-arena-accent">
              Awaiting your deposit · safe to cancel
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-5 sm:justify-end">
        <div className="text-right">
          ...
          <p className="font-display text-lg font-bold text-arena-text">
            {formatStake(match.stake_amount)} {SETTLEMENT_ASSET_LABEL}
          </p>
        </div>
        {isOwnChallenge ? (
