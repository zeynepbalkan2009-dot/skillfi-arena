...
export function PilotEnrollmentClient() {
  ...
  return <section className="rounded-2xl border border-cyan-300/20 bg-cyan-300/[.05] p-5">
    ...
    {enrollment ? <div className="mt-5 rounded-xl border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Application status</p><p className="mt-1 font-display text-2xl font-bold uppercase text-cyan-200">{enrollment.status}</p><p className="mt-2 text-sm text-slate-500">Submitted {new Date(enrollment.created_at).toLocaleDateString()}. Test access and testnet funding are granted only after manual review.</p></div> : <form onSubmit={submit} className="mt-5 space-y-3"><p className="text-sm leading-6 text-slate-400">Apply with your existing SkillFi profile. This pilot has no real deposits, prizes, lending, or production-value transfers.</p><Consent name="adult">I attest that I am at least 18 years old and eligible to participate where I live.</Consent><Consent name="terms">I accept the current pilot terms and understand this is a testnet product trial.</Consent><Consent name="privacy">I have read the privacy notice and consent to pilot telemetry and anonymized aggregate reporting.</Consent><button disabled={busy} className="mt-2 rounded-lg bg-cyan-300 px-5 py-3 text-sm font-black text-[#071014] disabled:opacity-50">{busy ? "SUBMITTING…" : authenticated ? "APPLY FOR BETA" : "SIGN IN TO APPLY"}</button></form>}
