create table if not exists public.risk_stake_reservations (
...
create index if not exists idx_risk_reservations_user_created
  on public.risk_stake_reservations (user_id, created_at desc);

alter table public.risk_stake_reservations enable row level security;
revoke all on public.risk_stake_reservations from anon, authenticated;
grant select, insert, update on public.risk_stake_reservations to service_role;
...
begin
  ...
  if found then
    ...
    return query select true, 'existing reservation', 0::numeric, 0::numeric;
    return;
  end if;
