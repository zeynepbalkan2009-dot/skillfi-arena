...
export async function GET(request: NextRequest) {
  ...
  if (!studio) return NextResponse.json({ credentials: [] }, { headers: PRIVATE_NO_STORE });
  ...
  if (error) return NextResponse.json({ error: "Could not load integration credentials" }, { status: 500, headers: PRIVATE_NO_STORE });
  return NextResponse.json({ credentials: data ?? [] }, { headers: PRIVATE_NO_STORE });
}
...
export async function POST(request: NextRequest) {
  ...
  if (!body?.gameId || name.length < 2 || name.length > 80) return NextResponse.json({ error: "gameId and a 2-80 character key name are required" }, { status: 400, headers: PRIVATE_NO_STORE });
  const { data: studio } = await supabaseAdmin.from("studios").select("id,status").eq("owner_user_id", user.id).maybeSingle();
  if (!studio || !['pending_review', 'approved'].includes(studio.status)) return NextResponse.json({ error: "Studio is not eligible for integration credentials" }, { status: 403, headers: PRIVATE_NO_STORE });
  ...
  if (!game || !['sandbox', 'published'].includes(game.integration_status)) return NextResponse.json({ error: "Credentials require a sandbox or published game" }, { status: 409, headers: PRIVATE_NO_STORE });
  ...
  if (error) return NextResponse.json({ error: "Could not create integration credential" }, { status: 500, headers: PRIVATE_NO_STORE });
  ...
  return NextResponse.json(
    { credential, secret: generated.secret, warning: "Copy this key now. It will not be shown again." },
    { status: 201, headers: { ...PRIVATE_NO_STORE, Pragma: "no-cache", Expires: "0" } },
  );
}
...
export async function PATCH(request: NextRequest) {
  ...
  if (!body?.credentialId) return NextResponse.json({ error: "credentialId is required" }, { status: 400, headers: PRIVATE_NO_STORE });
  const { data: studio } = await supabaseAdmin.from("studios").select("id").eq("owner_user_id", user.id).maybeSingle();
  ...
  if (error || !credential) return NextResponse.json({ error: "Credential is missing or already revoked" }, { status: 409, headers: PRIVATE_NO_STORE });
  ...
  return NextResponse.json({ credential }, { headers: PRIVATE_NO_STORE });
}
