...
export async function POST(request: NextRequest) {
  ...
  if (!body?.gameId || !body.stakeAmount) return NextResponse.json({ error: "gameId and stakeAmount are required" }, { status: 400 });

  let stake: bigint;
  try { stake = BigInt(body.stakeAmount); if (stake <= 0n) throw new Error(); } catch { return NextResponse.json({ error: "stakeAmount must be a positive integer in token base units" }, { status: 400 }); }
  ...
  const hash = await escrowWalletClient.writeContract({ address: ESCROW_CONTRACT_ADDRESS, abi: skillFiEscrowAbi, functionName: "createMatch", args: [matchId, stake, user.wallet_address as `0x${string}`] });
  const receipt = await escrowPublicClient.waitForTransactionReceipt({ hash });
  if (receipt.status !== "success") return NextResponse.json({ error: "On-chain match creation reverted" }, { status: 502 });

  const { data: match, error: insertError } = await supabaseAdmin.from("matches").insert({
    smart_contract_match_id: matchId.toString(), game_id: game.id, player_a_id: user.id, player_b_id: null, stake_amount: stake.toString(), status: "waiting_on_chain",
  }).select("id, smart_contract_match_id, game_id, player_a_id, player_b_id, stake_amount, status").single();
  if (insertError) return NextResponse.json({ error: "On-chain match exists but database indexing failed", txHash: hash }, { status: 502 });
